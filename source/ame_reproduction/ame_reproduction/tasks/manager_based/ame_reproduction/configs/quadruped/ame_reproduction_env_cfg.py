...
import ame_reproduction.tasks.manager_based.ame_reproduction.mdp as mdp
from isaaclab_tasks.utils import PresetCfg, preset

##
# Pre-defined configs
##
from ame_reproduction.tasks.manager_based.ame_reproduction.terrains import ROUGH_TERRAINS_CFG  # isort: skip
from ame_reproduction.tasks.manager_based.ame_reproduction.assets.robots.anymal import ANYMAL_D_CFG
...
class ObservationsCfg:
    ...
    class PolicyCfg(ObsGroup):
        ...
        def __post_init__(self):
            self.enable_corruption = False   # Enable observation noise (default False)
...
class RewardsCfg:
    ...
    track_ang_vel_z_world_exp = RewTerm(
        func=mdp.track_ang_vel_z_world_exp, weight=3.0, params={"command_name": "base_velocity", "std": math.sqrt(1.0)}
    )
    termination_penalty = RewTerm(
        func=mdp.is_terminated, weight=-200
    )
    ...
    dof_torque_limits = RewTerm(func=mdp.joint_torque_limits, weight=-0.2, params={"soft_ratio": 0.8})

    # -- styles
    lin_vel_z_l2 = RewTerm(func=mdp.lin_vel_z_l2, weight=-1.0)
    ang_vel_xy_l2 = RewTerm(func=mdp.ang_vel_xy_l2, weight=-5.0e-2)
...
@configclass
class AmeReproductionEnvCfg(ManagerBasedRLEnvCfg):
    """Configuration for the AME velocity-tracking environment."""
