...
All the environments are registered in the `ame_reproduction` extension. They start
...
with contextlib.suppress(ImportError):
    import ame_reproduction.tasks_experimental  # noqa: F401
...
def _format_presets(preset_map: dict | None) -> str:
    ...
    Args:
        preset_map: Mapping of :class:`~ame_reproduction.utils.preset_target.PresetTarget`
    ...
    if preset_map is None:
        return "(unavailable)"
    from ame_reproduction.utils.preset_target import PresetTarget
...
def main():
    """Print all environments registered in `ame_reproduction` extension."""
    ...
    if args_cli.show_presets:
        from ame_reproduction.utils.preset_cli import enumerate_task_presets
